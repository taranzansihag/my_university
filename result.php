...
<script src="university/jquery.js"></script>
  <script>
	...
	<nav class="w3-downnav w3-blue" style="display:none" id="mySidenav">
		...
		<b class="w3-text-shadow">RESULTS</b></div>
	</div>
	
	
		<img src="university/images/result1.jpg" class="img-responsive" width="100%">
	<br><br>
		...
		<div class="col-sm-6">
					<select name="result" class="form-control">
							<option>select type of result</option>
		<?php
			include("database.php");
			$rs=mysql_query("select * from nsresult");
			while($r=mysql_fetch_array($rs)){
				?>
					<option value="<?=$r[1]?> <?=$r[3]?> <?=$r[4]?>"><?=$r[1]?> <?=$r[3]?> year <?=$r[4]?> sem</option>
					
				<?php
			}
			include("database.php");
			$rs=mysql_query("select * from nbresult");
			while($r=mysql_fetch_array($rs)){
				?>
			...
			include("database.php");
			$rs=mysql_query("select * from nresult");
			while($r=mysql_fetch_array($rs)){
				?>
					<option value="<?=$r[1]?> <?=$r[2]?>"><?=$r[1]?> <?=$r[2]?> year</option>
